...
def get_coverage_cmd(input_filepaths, output_filepaths, output_directory, directories, opts):
    ...
    cmd += [
        ...
        # ">",
        # output_filepaths[1],
        os.environ["maxbin_abundance_from_metabat2_coverage_file.py"],
...
def get_metabat2_cmd(input_filepaths, output_filepaths, output_directory, directories, opts, prefix, seed):
    cmd = [
        ...
        "echo 'No MAGs: {}: '".format(os.path.join(output_directory, "scaffolds_to_bins.tsv")),
        "$(cat",
...
def get_dastool_cmd(input_filepaths, output_filepaths, output_directory, directories, opts):
    cmd = [
        ...
        "&&"
        'export R_LIBS="{}/lib/R/library"'.format(os.environ["DAS_Tool"].replace('/bin/DAS_Tool', '')),
        "&&"
...
# MDMcleaner
def get_mdmcleaner_cmd(input_filepaths, output_filepaths, output_directory, directories, opts):
    cmd = [
        opts.cmd_opener, # Assumes '&&' at end of command
        ...
        'echo -e "db_type\tgtdb\ndb_basedir\t{}" > {}'.format(
            ...
            os.path.join(directories['tmp'], "mdmcleaner.config"),
        ),
        
            "&&",
        
        os.environ["mdmcleaner"],
        "clean",
        "-c {}".format(os.path.join(directories['tmp'], "mdmcleaner.config")),
        "-i {}".format(input_filepaths[0]), # MAGs
        ...
        "for F in {}/*/*_filtered_kept_contigs.fasta.gz; do".format(output_filepaths[0]),
            "P={}/$(basename ${{F%*_filtered_kept_contigs.fasta.gz}});".format(output_filepaths[1]),
            'echo "Copying files and predicting genes for $P"',
            "&&",
            "gunzip -c $F > $P.fa",
            "&&",
    ...
    if opts.bam is not None:
        cmd += [
            ...
            "SRC={}; DST={}; SRC=$(realpath --relative-to $DST $SRC); ln -sf $SRC $DST".format(
                    os.path.join(directories[("intermediate", "{}__featurecounts".format(step-3))], "featurecounts.orfs.tsv.gz"),
                    output_directory,
                ),
            ]
...
def create_pipeline(opts, directories, f_cmds):
    ...
    if opts.bam is not None:
        ...
        input_filepaths = [opts.coverage]

    output_filenames = ["coverage_metabat.tsv", "coverage_maxbin2_list.tsv"]
    ...
    for iteration in range(1, opts.n_iter+1):
        ...
        input_filepaths = [
            input_fasta,
            os.path.join(directories[("intermediate",  "1__coverage")], "coverage_maxbin2_list.tsv"),
        ]
        ...
        # i/o
        if opts.bam is not None:
            input_filepaths = [input_fasta, *opts.bam]
        else:
            input_filepaths = [input_fasta]
        ...
        # Info
        description = "Evaluation via MDMcleaner [Iteration={}]".format(iteration)
        # i/o
        input_filepaths = [
            os.path.join(directories[("intermediate",  "{}__dastool".format(steps["dastool"]))], "__DASTool_bins", "*.fa"),
            os.path.join(directories[("intermediate",  program_label)], "blacklist.txt"),
        ]

        output_filepaths = [
            os.path.join(directories[("intermediate",  program_label)], "__MDMcleaner_bins"),
            os.path.join(directories[("intermediate",  program_label)], "bins"),
        ]
        ...
        # i/o
        input_filepaths = [
            os.path.join(directories[("intermediate",  "{}__mdmcleaner".format(steps["mdmcleaner"]))], "bins"),
            input_fasta,
        ]
...
def add_executables_to_environment(opts):
    ...
    accessory_scripts = {
                ...
                "subset_table.py",
                "compile_gff.py",
                "maxbin_abundance_from_metabat2_coverage_file.py",
                }
...
def configure_parameters(opts, directories):
    ...
    assert any([bool(opts.bam), bool(opts.coverage)]), "Must have either --bam or --coverage for metabat2 (and maxbin2 if --skip_maxbin2 not used)"
    if not opts.skip_concoct:
        assert opts.bam is not None, "Must provide --bam if running concoct (i.e., --skip_concoct is not used)"
...
def main(args=None):
    ...
    parser_io.add_argument("-c","--coverage", type=str, required=False, help = "path/to/coverage.tsv. Can be used with metabat2 and maxbin2 but not concoct and cannot be used with --bam")
    ...
    parser_evaluation.add_argument("--mdmcleaner_options", type=str, default="", help="MDMcleaner | More options (e.g. --fast_run ) [Default: ''] | https://github.com/KIT-IBG-5/mdmcleaner")
    ...
    # opts.bam = [os.path.abspath(x) for x in opts.bam]
